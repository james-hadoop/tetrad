...
public final class Edges {
    ...
    public static void sortEdges(List<Edge> edges) {
        for (Edge edge : new ArrayList<>(edges)) {
            if (Edges.isUndirectedEdge(edge) || Edges.isNondirectedEdge(edge) || Edges.isBidirectedEdge(edge)) {
                int i = (edge.getNode1().toString()).compareTo((edge.getNode2().toString()));
                if (i > 0) {
                    ...
                    edges.remove(edge);
                    edges.add(new Edge(n2, n1, e2, e1));
                }
            }
        }
