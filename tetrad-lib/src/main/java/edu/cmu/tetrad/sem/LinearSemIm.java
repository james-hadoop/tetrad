 ...
 */
public final class LinearSemIm implements IM, ILinearSemIm, TetradSerializable {
     ...
     */
    private final LinearSemPm linearSemPm;
     ...
     */
    private final Map<Node, ConnectionFunction> functions;
     ...
     */
    public LinearSemIm(LinearSemPm linearSemPm) {
        this(linearSemPm, null, new Parameters());
    }
     ...
     */
    public LinearSemIm(LinearSemPm linearSemPm, Parameters params) {
        this(linearSemPm, null, params);
    }
     ...
     */
    public LinearSemIm(LinearSemPm linearSemPm, LinearSemIm oldSemIm, Parameters parameters) {
        if (linearSemPm == null) {
        ...
        this.linearSemPm = new LinearSemPm(linearSemPm);

        this.variableNodes
                = Collections.unmodifiableList(linearSemPm.getVariableNodes());
        this.measuredNodes
                = Collections.unmodifiableList(linearSemPm.getMeasuredNodes());
     ...
     * matrix.
     */
    public LinearSemIm(LinearSemPm linearSemPm, ICovarianceMatrix covMatrix) {
        this(linearSemPm);
     ...
     */
    private LinearSemIm(LinearSemIm semIm, Matrix covariances,
                        Vector means) {
        this(semIm);
        ...
        if (means.size() != linearSemPm.getVariableNodes().size()) {
            throw new IllegalArgumentException(
                    "Number of means does not equal " + "number of variables.");
        }

        if (covariances.rows() != linearSemPm.getVariableNodes().size()) {
            throw new IllegalArgumentException(
     ...
     */
    public LinearSemIm updatedIm(Matrix covariances, Vector means) {
        return new LinearSemIm(this, covariances, means);
    }
     ...
     */
    public LinearSemIm(LinearSemIm semIm) throws RuntimeException {
        try {
            LinearSemIm _semIm = new MarshalledObject<>(semIm).get();

            linearSemPm = _semIm.linearSemPm;
            ...
            sampleCovar = _semIm.sampleCovar == null ? null : _semIm.sampleCovar.copy();
            ...
            cyclicChecked = _semIm.cyclicChecked;
            params = _semIm.params;
     ...
     */
    public static LinearSemIm retainValues(LinearSemIm semIm, SemGraph graph) {
        LinearSemPm newLinearSemPm = new LinearSemPm(graph);
        LinearSemIm newSemIm = new LinearSemIm(newLinearSemPm);
     ...
     */
    public static LinearSemIm serializableInstance() {
        return new LinearSemIm(LinearSemPm.serializableInstance());
    }
    ...
    public void setCovMatrix(ICovarianceMatrix covMatrix) {
        ...
        this.sampleCovar = covMatrix2.getMatrix().copy();
    ...
    public double getErrVar(Node x) {
        Parameter param = linearSemPm.getVarianceParameter(x);
        return getParamValue(param);
    }

    public double getErrCovar(Node x, Node y) {
        Parameter param = linearSemPm.getCovarianceParameter(x, y);
        return getParamValue(param);
    }

    public double getEdgeCoef(Node x, Node y) {
        Parameter param = linearSemPm.getCoefficientParameter(x, y);
        return getParamValue(param);
    }
    ...
    public boolean existsEdgeCoef(Node x, Node y) {
        return x != y && linearSemPm.getCoefficientParameter(x, y) != null;
    }
    ...
    public double getIntercept(Node node) {
        node = linearSemPm.getGraph().getNode(node.getName());
...
//            TetradMatrix implCovar = getImplCovar();
    ...
    public double getRmsea() {
        double v = getChiSquare() - linearSemPm.getDof();
        double v1 = linearSemPm.getDof() * (getSampleSize() - 1);
    ...
    public double getCfi() {
        ...
        LinearSemIm nullIm = independenceModel();
    ...
    private DataSet simulateData(int sampleSize, DataSet initialValues, boolean latentDataSaved) {
        if (linearSemPm.getGraph().isTimeLagModel()) {
    ...
    private DataSet simulateTimeSeries(int sampleSize, boolean latentDataSaved) {
        SemGraph semGraph = new SemGraph(linearSemPm.getGraph());
        semGraph.setShowErrorTerms(true);
        TimeLagGraph timeSeriesGraph = linearSemPm.getGraph().getTimeLagGraph();
    ...
    public boolean isCyclic() {
        if (!cyclicChecked) {
            this.cyclic = linearSemPm.getGraph().existsDirectedCycle();
            cyclicChecked = true;
        }
    ...
    public String toString() {
        ...
        if (sampleCovar == null) {
            ...
            buf.append("\n\nsample cov:\n");
            buf.append(MatrixUtils.toString(sampleCovar.toArray()));
        }
    ...
    //==============================PRIVATE METHODS====================//
    private void retainPreviousValues(LinearSemIm oldSemIm) {
        if (oldSemIm == null) {
            System.out.println("old sem im null");
            return;
        }

        List<Node> nodes = linearSemPm.getGraph().getNodes();
    ...
    private List<Node> unmeasuredLatents(LinearSemPm linearSemPm) {
        SemGraph graph = linearSemPm.getGraph();
    ...
    private List<Parameter> initFreeParameters() {
        return Collections.unmodifiableList(linearSemPm.getFreeParameters());
    }
...
//        return det > 0 ? Math.log(det) : 0;
    ...
    public void setFunction(Node node, ConnectionFunction function) {
        List<Node> parents = linearSemPm.getGraph().getParents(node);
