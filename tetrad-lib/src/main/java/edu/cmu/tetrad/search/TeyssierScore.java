 ...
 */
public class TeyssierScore implements Score {
     ...
     */
    public TeyssierScore(ICovarianceMatrix covariances) {
        if (covariances == null) {
        ...
        this.scorer = new TeyssierScorer(new SemBicScore(covariances));
     ...
     * Constructs the score using a covariance matrix.
     */
    public TeyssierScore(DataSet dataSet) {
        if (dataSet == null) {
            throw new NullPointerException();
        }
    ...
    public double localScore(int i, int... parents) {
        ...
        return scorer.getParents(order.size() - 1).size();
    }
