 ...
 */
public class LinearSemImWrapper implements SessionModel {
    ...
    //============================CONSTRUCTORS==========================//
    public LinearSemImWrapper(LinearSemIm semIm) {
        setSemIm(semIm);
    }

    public LinearSemImWrapper(SemEstimatorWrapper semEstWrapper) {
        if (semEstWrapper == null) {
            throw new NullPointerException();
        }

        LinearSemIm oldSemIm = semEstWrapper.getSemEstimator().getEstimatedSem();

        try {
            setSemIm((LinearSemIm) new MarshalledObject(oldSemIm).get());
        } catch (Exception e) {
            throw new RuntimeException("SemIm could not be deep cloned.", e);
        }
    }

    public LinearSemImWrapper(Simulation simulation) {
        if (simulation == null) {
    ...
    public LinearSemImWrapper(LinearSemPmWrapper semPmWrapper, Parameters params) {
        if (semPmWrapper == null) {
            throw new NullPointerException("SemPmWrapper must not be null.");
        }

        setSemIm(new LinearSemIm(semPmWrapper.getSemPms().get(semPmWrapper.getModelIndex()), params));
    }

    public LinearSemImWrapper(LinearSemPmWrapper semPmWrapper, LinearSemImWrapper oldLinearSemImWrapper,
                              Parameters params) {
        if (semPmWrapper == null) {
            throw new NullPointerException("SemPmWrapper must not be null.");
        }
        ...
        LinearSemPm linearSemPm = new LinearSemPm(semPmWrapper.getSemPm());
        LinearSemIm oldSemIm = oldLinearSemImWrapper.getSemIm();

        if (!params.getBoolean("retainPreviousValues", false)) {
            setSemIm(new LinearSemIm(linearSemPm, params));
        } else {
            setSemIm(new LinearSemIm(linearSemPm, oldSemIm, params));
        }
    }

    public LinearSemImWrapper(SemUpdaterWrapper semUpdaterWrapper) {
    ...
    private void setSemIm(LinearSemIm updatedSemIm) {
        semIms = new ArrayList<>();
        semIms.add(new LinearSemIm(updatedSemIm));
    ...
    public LinearSemImWrapper(LinearSemImWrapper semImWrapper) {
    ...
    public LinearSemImWrapper(HbsmsWrapper wrapper) {
        LinearSemIm oldSemIm = wrapper.getNewSemIm();
        setSemIm(oldSemIm);
    }
...
//                        new Parameters()), new Parameters());
    ...
    //======================== Private methods =======================//
    private void log(int i, LinearSemIm pm) {
